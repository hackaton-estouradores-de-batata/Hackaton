...
| Etapa | Entrega |
|------|---------|
| 1. Ingestão | `POST /api/cases` recebe multipart, cria registro em `cases` e salva PDFs em `data/processos_cadastrados/case_<id>/` |
...
| Tarefa | Arquivo |
|--------|---------|
| EDA do CSV: distribuição de `valor_causa`, `resultado_macro`, `resultado_micro`, `valor_condenacao` | `src/scripts/analyze_historical.py` |
| Consolidar mapa nacional de desconto efetivo sobre o PED por `Resultado micro` | `src/scripts/analyze_historical.py` |
| Conferir cobertura por `UF` e `Sub-assunto` para suportar as matrizes da V5 | `src/scripts/analyze_historical.py` |
...
| Corrigir a leitura de êxito/não êxito usando `Resultado micro` como prioridade, sem depender só de `Resultado macro` | P2 | `src/scripts/analyze_historical.py` |
...
| Tarefa | Responsável | Arquivo |
|--------|-------------|---------|
| Criar `eval_policy.py`: aplicar a V5 no histórico elegível e reportar mix `acordo/defesa`, economia estimada, custo esperado e distribuição por `UF`/`Sub-assunto` | P2 | `src/scripts/eval_policy.py` |
| Rodar cenários por `qtd_docs` (`0..6`), já que o CSV não traz os PDFs nem o inventário documental real | P2 | `src/scripts/eval_policy.py` |
| Separar `casos elegiveis` de `casos inconclusivos` no relatório final | P2 | `src/scripts/eval_policy.py` |
