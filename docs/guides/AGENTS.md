...
The `hackathon-ufmg-2026/` path is tracked as a subproject pointer and may be empty in this clone, so coordinate before changing its state. When implementation begins, follow the planned structure in `PROJECT_SKELETON.md`: `src/web` for Next.js, `src/api` for FastAPI, `src/policy` for agreement rules, `src/scripts` for offline tooling, plus `data/` and `docs/`.
